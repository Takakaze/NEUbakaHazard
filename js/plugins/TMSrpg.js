...
(function() {
  ...
  Game_Map.prototype.createEffectArea = function(x, y, skill) {
    ...
    Game_Map.prototype.createArea = function (x, y, param) {
        ...
        }else if (param[0] === 'Conical') {
            this.setArea(area, this.isAreaValidConical, x, y, +param[1], true);
        }
  ...
  Game_Map.prototype.isAreaValidLine = function(x, y, x2, y2, size) {
    ...
    Game_Map.prototype.isAreaValidConical = function (x, y, x2, y2, size) {
        var character = $gamePlayer;
        //    var character = this._srpgStatusWindow.user();
        if (character.direction() == 8) {
            return (Math.abs(x - x2) + Math.abs(y - y2) <= size && y >= y2);
        }
        else if (character.direction() == 2) {
            return (Math.abs(x - x2) + Math.abs(y - y2) <= size && y <= y2);
        }
        else if (character.direction() == 4) {
            return (x >= x2 && Math.abs(x - x2) + Math.abs(y - y2) <= size);
        }
        else if (character.direction() == 6) {
            return (x <= x2 && Math.abs(x - x2) + Math.abs(y - y2) <= size);
        }
  ...
  Game_Event.prototype.executeSrpgWaiting = function(x, y) {
     ...
     // if (this._moved == false && this._acted == false) {
...
//      }
    ...
    var subject = this.srpgBattler();
      ...
      if (subject.isActor() === true) {
          if (subject.actorId() === 1) {
              var skill = $dataSkills[6];
          }
          else if (subject.actorId() === 6) {
              var skill = $dataSkills[7];
          }
          else if (subject.actorId() === 8) {
              var skill = $dataSkills[9];
          }
          else if (subject.actorId() === 9) {
              var skill = $dataSkills[10];
          }
          else if (subject.actorId() === 10) {
              var skill = $dataSkills[8];
          }
          else if (subject.actorId() === 4) {
              var skill = $dataSkills[138];
          }
          else {
  ...
  Game_Event.prototype.srpgCheckCounter = function(action, attacker) {
    var battler = this.srpgBattler();
    if (battler.isActor()) {
        $gameVariables.setValue(8, battler.actorId());
        $gameVariables.setValue(32, battler.level);
    }
    else {
      ...
      if (Math.random() < action.itemCnt(battler)) {
          if (battler.isActor() == true) {
              if (battler.actorId() == 1) {
                  var skill = $dataSkills[6];
              }
              else if (battler.actorId() == 6) {
                  var skill = $dataSkills[7];
              }
              else if (battler.actorId() == 8) {
                  var skill = $dataSkills[9];
              }
              else if (battler.actorId() == 9) {
                  var skill = $dataSkills[10];
              }
              else if (battler.actorId() == 10) {
                  var skill = $dataSkills[8];
              }
              else {
  ...
  Window_SrpgCommand.prototype.makeCommandList = function() {
    ...
    if (this._srpgUnit.isSrpgInputtable()) {
      ...
      if (canMove || (this._srpgUnit.isLastPositionValid() == false && canAct == true)) { 
      ...
      if (equipCommand) this.addCommand(equipCommand, 'equip', (canMove || (this._srpgUnit.isLastPositionValid() == false && canAct)));
      if (waitingCommand) {
  ...
  Window_SrpgStatus.prototype.makeEffectText = function(targetBattler, effect, hit) {
    ...
    case Game_Action.EFFECT_ADD_DEBUFF:       // 弱体
      var buffId = effect.dataId;
      return TextManager.param(buffId) + '弱化' + effect.value1 + '回合' + '(' + hit + '%)';
    ...
    case Game_Action.EFFECT_REMOVE_DEBUFF:    // 弱体解除
      var buffId = effect.dataId;
      return TextManager.param(buffId) + '弱化解除' + '(' + hit + '%)';
...
//
  ...
  Window_SrpgRegionEffect.prototype.refresh = function() {
    ...
    this.contents.clear();
    this.drawTextEx('\\C[17]\\}地形\\{\\C[3]', 0, 0)
    this.drawText(state ? state.name : '无地形', 44, 0, this.contents.Width - 48);
  };
  ...
  Scene_Map.prototype.updateAreaSelecter = function() {
    ...
    Scene_Map.prototype.updateAreaSelecter2 = function () {
        if (Input.isPressed('ok')) {
            var x = $gameMap.canvasToMapX(Input.x);
            var y = $gameMap.canvasToMapY(Input.y);
            if ($gameMap.isInsideArea(x, y)) this.callAreaOkHandler(x, y);
  ...
  Scene_Map.prototype.updateSrpgInput = function() {
    ...
    // SRPG入力の更新
    Scene_Map.prototype.updateSrpgInput2 = function () {    
        var event = $gameMap.srpgUnitXy(x, y, true);
        if (event && Input.isPressed('ok')) {
            this._srpgTurnState = 0;
            this.openSrpgCommand(event, true);  // ユニットコマンドを開く
            SoundManager.playOk();              // 決定効果音を鳴らす
            $gamePlayer.setSrpgCameraXy(x, y);    // カメラ移動
  ...
  Scene_Map.prototype.okAreaAction = function(x, y) {
    ...
    Scene_Map.prototype.setSrpgAction = function (event, skill, x, y) {
        ...
        this.setSrpgAction2(event, skill, x, y);
    };

    Scene_Map.prototype.setSrpgAction2 = function (event, skill, x, y) {
        $gameMap.showEffectArea(x, y, skill);       // 効果範囲の表示
        event.turnTowardXy(x, y);                   // 実行先の座標を向く
        $gamePlayer.setSrpgCameraXy(x, y);          // カメラ移動
